...
public class VaccinationsReadService extends AbstractReadService {
	...
	public List<VaccinationDto> getVaccinationList(String patientIamId) {
		...
		return bundle.getEntry().stream().filter(e -> e.getResource() instanceof org.hl7.fhir.r4.model.Composition)
				...
				.filter(section -> section.getCode().getCoding().stream()
						.anyMatch(coding -> "http://loinc.org".equals(coding.getSystem())
								&& IMMUNIZATION_LOINC_CODE.equals(coding.getCode())))
				.flatMap(section -> section.getEntry().stream()).map(ref -> resolveImmunization(ref, immunizationMap))
				.filter(Objects::nonNull).map(imm -> {
	...
	static Map<String, String> buildPractitionerRoleDisplayMap(Bundle bundle) {
		...
		for (var entry : bundle.getEntry()) {
			if (entry.getResource() instanceof PractitionerRole pr) {
				...
				if (entry.getFullUrl() != null) {
					roleDisplayMap.put(entry.getFullUrl(),
							resolveNameFromPracRef(pr, pracRefToDisplay, entry.getFullUrl()));
				}
	...
	private static String resolveNameFromPracRef(PractitionerRole pr, Map<String, String> pracRefToDisplay,
			String fallbackKey) {
	...
	private void resolvePractitionerDisplay(Immunization imm, Map<String, String> practitionerRoleDisplayMap) {
		for (var performer : imm.getPerformer()) {
			...
			if (actor == null || actor.hasDisplay())
				continue;
